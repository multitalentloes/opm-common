...
{
public:
    OPM_HOST_DEVICE WaterPvtMultiplexer()
        : approach_(WaterPvtApproach::NoWater)
        , realWaterPvt_(nullptr)
    {
    }

    OPM_HOST_DEVICE WaterPvtMultiplexer(WaterPvtApproach approach, void* realWaterPvt)
        : approach_(approach)
        , realWaterPvt_(realWaterPvt)
    { }

    OPM_HOST_DEVICE WaterPvtMultiplexer(const WaterPvtMultiplexer<Scalar,enableThermal,enableBrine>& data)
    {
        *this = data;
    }
    ...
    template <WaterPvtApproach approachV>
    OPM_HOST_DEVICE typename std::enable_if<approachV == WaterPvtApproach::BrineCo2, BrineCo2Pvt<Scalar> >::type& getRealPvt()
    {
        assert(approach() == approachV);
    ...
    template <WaterPvtApproach approachV>
    OPM_HOST_DEVICE typename std::enable_if<approachV == WaterPvtApproach::BrineCo2, const BrineCo2Pvt<Scalar> >::type& getRealPvt() const
    {
        assert(approach() == approachV);
    ...
    OPM_HOST_DEVICE const void* realWaterPvt() const { return realWaterPvt_; }
...
namespace gpuistl{
    ...
    copy_to_gpu(const WaterPvtMultiplexer<Scalar>& cpuWaterPvt)
    {

        assert(WaterPvtApproach::BrineCo2 == cpuWaterPvt.WaterPvtApproach);
